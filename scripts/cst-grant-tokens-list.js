...
const optionsDefs = [
  ...
  { name: "raw", type: Boolean, description: "Use the token amounts in the CSV as the actual token amounts without coverting using the contract decimals field"},
...
module.exports = async function(callback) {
  ...
  await Parallel.each(rows, async row => {
    ...
    if (address && amount && amount.trim()) {
      if (raw) {
        console.log(`Granting ${address} raw token amount ${amount}`);
        try {
          await cst.grantTokens(address, amount.trim());
        } catch (err) {
