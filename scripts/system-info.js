...
module.exports = async function(callback) {
  ...
  let cst, cstRegistry, cstFrozen, cstStorageName, cstLedgerName, cstName,
    ...
    registryVersion, registryOwner, ledgerVersion, storageOwner, ledgerOwner, cstOwner;
  ...
  try {
    ...
    if (ledgerAddress === NULL_ADDRESS) {
      ...
      let ledgerData = cst ? await getLedger(web3, cst) : {};
      numAccounts = Object.keys(ledgerData).length;

      try {
        ledgerVersion = await ledger.version();
      } catch (err) {
        ledgerVersion = "1"; // this property wasnt introduced in the initial contracts
      }
    }
  } catch (err) {
    console.error("Encountered error getting token details", err);
