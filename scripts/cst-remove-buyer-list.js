...
const usage = [
  {
    header: "cst-remove-buyer-list",
    content: "This script removes buyers from a list of ethereum addresses to the CST buyers whitelist."
  },{
    header: "Options",
    optionList: [{
      name: "help",
...
module.exports = async function(callback) {
  ...
  console.log(`Scheduling ${rows.length} buyers to be removed from the CST contract ${cst.address}.`);
  ...
  await Parallel.each(rows, async row => {
    ...
    let [ address ] = row.replace(/"/g, "").split(",");
    try {
      await cst.removeBuyer(address.trim());
    } catch (err) {
      console.error(`Error encountered removing buyer ${address}, ${err.message}`);
    }
  }, concurrency);
