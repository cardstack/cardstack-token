...
contract("EthToUsdOracle", function(accounts) {
  ...
  it("Callback function cannot be invoked by non-oraclize address", async function() {
    let owner = accounts[0];
    let oracleBalance = web3.toWei(0.1, "ether");
    let oracle = await EthToUsdOracle.new(true, {
      from: owner,
  ...
  it("Non-owner should not be able to change the update frequency", async function() {
    let owner = accounts[0];
  ...
  it("Cannot set update frequency to 0 sec", async function() {
    let owner = accounts[0];
    let oracleBalance = web3.toWei(0.1, "ether");
    let oracle = await EthToUsdOracle.new(true, {
      from: owner,
