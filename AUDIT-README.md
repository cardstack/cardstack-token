...
Note that the CST contract is not able to be used until it is "registered" with the registry. After the CST contract has been registered, which bascially amounts to assigning the contract a "name" and resolving it's declared contract depenencies, it will then be able to be used.
...
It should also be noted that the registry itself is an "upgradable" contract.
